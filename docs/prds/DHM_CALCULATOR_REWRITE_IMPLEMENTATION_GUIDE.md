# DHM Dosage Calculator Implementation Guide - Zero Ambiguity Version

## Overview
This is a prescriptive, step-by-step guide to rewrite the DHM Dosage Calculator. Follow exactly as written. No variations allowed.
...
```
src/pages/DosageCalculatorRewrite/
├── index.jsx
├── CalculatorForm.jsx
...
Create `src/pages/DosageCalculatorRewrite/utils.js`:
...
## Step 2: Create CalculatorForm.jsx

Create `src/pages/DosageCalculatorRewrite/CalculatorForm.jsx`:
...
## Step 3: Create CalculatorResults.jsx
...
## Step 4: Create EducationSection.jsx
...
## Step 5: Create index.jsx (Main Component)
...
In `src/App.jsx`, make these EXACT changes:

### 6.1: Add Import
Find the section with lazy imports (look for lines starting with `const` and containing `lazy`).
It should be around lines 6-14. Add this line with the other imports:
...
### 6.2: Add Route
Find the `renderPage()` function (search for "function renderPage" or "const renderPage").
Inside the switch statement, BEFORE the `default:` case, add:
...
switch (currentPath) {
  case '/guide':
    return <Guide />
  // ... other cases ...
  case '/dhm-dosage-calculator-new':  // ADD THIS
    return <DosageCalculatorRewrite />  // ADD THIS
  default:
    return <Home />
}
```
...
1. Navigate to `/dhm-dosage-calculator-new`
2. Test all form inputs
3. Verify calculations match the formula
4. Check mobile responsiveness
5. Confirm no z-index issues
6. Verify navigation is not blocked

## Step 8: Migration Plan

1. Deploy to `/dhm-dosage-calculator-new` first
2. Test for 48 hours
3. Monitor engagement metrics
4. If successful, create redirect from `/dhm-dosage-calculator` to `/dhm-dosage-calculator-new`
5. After 7 days with no issues, remove old calculator

## Required NPM Packages

All packages are already installed. Do NOT install anything new.

## Styling Classes Already Available

From `calculator-enhancements.css`:
- `animate-pulse-glow` - for glowing effect on badges
- `hover:shadow-lg` - for card hover effects
- `transition-all duration-300` - for smooth transitions

## Do NOT:
- Create any new CSS files
- Add any inline styles beyond what's shown
- Install new packages
- Create custom hooks
- Modify the calculation formula
- Add features not specified
- Change component structure
- Use different UI components
- Add custom animations beyond shown
- Create new utility functions

## Verification Checklist

- [ ] All 5 component files created exactly as shown
- [ ] App.jsx updated with new route
- [ ] No custom CSS created
...
This guide is complete and prescriptive. Follow it exactly with no deviations.